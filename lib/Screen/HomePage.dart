...
import 'package:tlkmartuser/Model/Deal_Model.dart';
...
class _HomePageState extends State<HomePage>
  ...
  Widget _getDeals() {
    return Selector<HomeProvider, bool>(
      selector: (_, homeProvider) => homeProvider.dealsLoading,
              ...
              3,
                ...
                if (index < dealsList.length) {
                  return Container(
                    ...
                    child: dealsTile(dealsList[index]),
                  );
                } else {
  ...
  //                     return Container(
  //                         height: MediaQuery.of(context).orientation ==
  //                                 Orientation.portrait ? 50 : deviceHeight! * 0.2,
  //                         width: deviceWidth! * 0.95,
  //                         child: dealsTile(dealsList[index]));
  //                   },
  ...
  Widget dealsTile(DealModel dealmodel) {
    print(imageUrl + (dealmodel.image ?? ''));
    return Container(
      height: 70,
      child: Row(
          ...
          Container(
              ...
              decoration: BoxDecoration(
                image: DecorationImage(
                    image: NetworkImage(imageUrl + (dealmodel.image ?? '')),
                    fit: BoxFit.fill),
              )
  ...
  void getDeals() {
    Map<String, String> parameter = {MASTER_TAB: isSelectedTab.toString()};
    apiBaseHelper.postAPICall(getDealsApi, parameter).then((getdata) async {
      bool error = getdata["error"];
      String? msg = getdata["message"];
      if (!error) {
        var data = getdata["data"];
        ...
        dealsList =
            (data as List).map((data) => DealModel.fromJson(data)).toList();
      } else {
        setSnackbar(msg!, context);
      }
